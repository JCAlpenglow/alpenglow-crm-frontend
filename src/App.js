  ...
  return (
    <div style={S.loginWrap}>
      <div style={S.loginCard}>
        ...
        {error && <div style={{ background: 'rgba(196,82,42,0.15)', border: '1px solid rgba(196,82,42,0.3)', borderRadius: 3, padding: '8px 12px', fontSize: 11, color: '#E07858', marginBottom: 14 }}>{error}</div>}
              ...
              {searchOpen && searchResults.length > 0 && (
                <div style={S.sDrop}>
                  ...
                  {searchResults.map(r => {
                    ...
                    return (
                      <div key={r.id} style={S.sRow} onClick={() => handleSearchSelect(r)}
                        onMouseEnter={e => e.currentTarget.style.background = 'rgba(196,82,42,0.1)'}
                        onMouseLeave={e => e.currentTarget.style.background = 'transparent'}>
                        <div style={{ ...S.sIco, background: avatarStyle(r.full_name).b, color: avatarStyle(r.full_name).c }}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 3 }}>
                          <span style={{ fontSize: 8, letterSpacing: '0.09em', textTransform: 'uppercase', padding: '2px 6px', borderRadius: 2, fontWeight: 500, ...(S[STAGE_CLS[r.stage]] || S.bok) }}>{r.stage}</span>
...
function ContactModal({ modal, profile, onClose, onSave, onDelete, onMoveStage, onPromotionRequest, onPromotionApprove, showToast }) {
  ...
  if (modal.type === 'promote') {
    return (
      <div style={S2.ovl} onClick={e => e.target === e.currentTarget && onClose()}>
        <div style={{ ...S2.modal, borderColor: '#C9A84C55' }}>
          <div style={S2.mHdr}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <i className="ti ti-lock" style={{ fontSize: 16, color: '#C9A84C' }} />
              <span style={{ ...S2.mTitle, color: '#C9A84C' }}>Promote to Invested</span>
            </div>
            <button style={S2.ibtn} onClick={onClose}><i className="ti ti-x" /></button>
          </div>
          ...
          <div style={S2.fGrid}>
            <div style={S2.fGrp}>
              <label style={S2.fLbl}>Share class</label>
              <select style={S2.fInp} value={promoDetails.share_class} onChange={setP('share_class')}>
                <option value="A">Class A</option>
                <option value="P">Class P</option>
              </select>
            </div>
            <div style={S2.fGrp}>
              <label style={S2.fLbl}>Commitment amount ($)</label>
              <input style={S2.fInp} type="number" placeholder="2500000" value={promoDetails.commitment_amount} onChange={setP('commitment_amount')} />
            </div>
            <div style={{ ...S2.fGrp, ...S2.fFull }}>
              <label style={S2.fLbl}>Accreditation status</label>
              <select style={S2.fInp} value={promoDetails.accreditation_status} onChange={setP('accreditation_status')}>
            ...
            <div style={{ ...S2.fGrp, ...S2.fFull }}>
              <label style={S2.fLbl}>Notes</label>
              <textarea style={S2.fTa} value={promoDetails.notes} onChange={setP('notes')} placeholder="KYC notes, wire details reference, any relevant context…" />
            </div>
          </div>
          <div style={S2.mFt}>
            <button style={S2.btn} onClick={onClose}>Cancel</button>
            <button style={{ ...S2.btnP, background: '#C9A84C', borderColor: '#C9A84C' }}
        ...
        <div style={S2.modal}>
          <div style={S2.mHdr}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 9 }}>
              <div style={{ ...S2.av, ...av, width: 28, height: 28, fontSize: 9 }}>{initials(c.full_name)}</div>
              <span style={S2.mTitle}>{c.full_name}</span>
            </div>
            <div style={{ display: 'flex', gap: 4 }}>
              <button style={S2.ibtn} onClick={() => onClose() || undefined}><i className="ti ti-edit" style={{ fontSize: 13 }} /></button>
              <button style={S2.ibtn} onClick={onClose}><i className="ti ti-x" /></button>
            </div>
          </div>
          {[
          ...
          <div style={{ display: 'flex', gap: 10, alignItems: 'flex-start', marginBottom: 14, fontSize: 12 }}>
            ...
            <span style={S2[b.cl]}>{c.last_contact_date} · {b.lb}</span>
          </div>
          ...
          <textarea style={S2.fTa} defaultValue={c.notes} />
          <div style={S2.mFt}><button style={S2.btn} onClick={onClose}>Close</button></div>
        </div>
      </div>
    );
  }
  ...
  return (
    <div style={S2.ovl} onClick={e => e.target === e.currentTarget && onClose()}>
      <div style={S2.modal}>
        <div style={S2.mHdr}>
          <span style={S2.mTitle}>{modal.type === 'edit' ? 'Edit Contact' : 'New Contact'}</span>
          <button style={S2.ibtn} onClick={onClose}><i className="ti ti-x" /></button>
        </div>
        <div style={S2.fGrid}>
          {[
            ...
            <div key={key} style={S2.fGrp}>
              <label style={S2.fLbl}>{label}</label>
              <input style={S2.fInp} type={type} placeholder={placeholder} value={form[key]} onChange={set(key)} />
            </div>
          ))}
          <div style={S2.fGrp}>
            <label style={S2.fLbl}>Contact type</label>
            <select style={S2.fInp} value={form.type} onChange={set('type')}>
          ...
          <div style={S2.fGrp}>
            <label style={S2.fLbl}>Source</label>
            <select style={S2.fInp} value={form.source} onChange={set('source')}>
              {SOURCES.map(s => <option key={s} value={s}>{s}</option>)}
            </select>
          </div>
          <div style={S2.fGrp}>
            <label style={S2.fLbl}>Stage</label>
            <select style={S2.fInp} value={form.stage} onChange={set('stage')}>
              {STAGES.map(s => <option key={s.id} value={s.id}>{s.lb}</option>)}
            </select>
          </div>
          <div style={S2.fGrp}>
            <label style={S2.fLbl}>Last contact</label>
            <input style={S2.fInp} type="date" value={form.lastContact} onChange={set('lastContact')} />
          </div>
          <div style={{ ...S2.fGrp, ...S2.fFull }}>
            <label style={S2.fLbl}>Notes</label>
            <textarea style={S2.fTa} value={form.notes} onChange={set('notes')} />
          </div>
        </div>
        <div style={S2.mFt}>
          <button style={S2.btn} onClick={onClose}>Cancel</button>
          <button style={S2.btnP} onClick={() => onSave(form, modal.contact?.id)}>
